class VDOM {
	constructor(data) {
		const nodes = data.split("\n")
		Object.assign(this, {
			data,  // The plain text data.
			nodes, // The parsed nodes.
		})
	}
	// `read` reads the VDOM.
	read(pos1 = 0, pos2 = this.data.length) { // Arguments can be omitted.
		return this.data.slice(pos1, pos2)
	}
	// `write` writes to the VDOM.
	write(data, pos1, pos2) {

		// Start node and offset:
		let pos = pos1
		let node1 = 0
		let offset1 = 0
		while (node1 < this.nodes.length) {
			if (pos - this.nodes[node1].length <= 0) {
				// Offset from the start:
				offset1 = pos
				break
			}
			pos -= this.nodes[node1].length
			if (node1 + 1 < this.nodes[node1].length) {
				pos--
			}
			node1++
		}

		// End node and offset (inverted):
		pos = pos2 - pos1
		let node2 = node1
		let offset2 = 0
		while (node2 < this.nodes.length) {
			if (pos - this.nodes[node2].length <= 0) {
				// Offset from the end:
				offset2 = pos - this.nodes[node2].length
				break
			}
			pos -= this.nodes[node2].length
			if (node2 + 1 < this.nodes[node2].length) {
				pos--
			}
			node2++
		}

		const range = {
			node: [node1, node2 + 1],
			offset: [offset1, offset2 + 1],
		}

		const newData = this.data.slice(0, pos1) + data + this.data.slice(pos2)
		const newNodes = []

		// Start node; must have one or more nodes:
		const nodes = data.split("\n")
		if (nodes.length) {
			newNodes.push(this.nodes[range.node[0]].slice(0, range.offset[0]) + nodes[0])
		}
		// Intermediary nodes; must have three or more nodes:
		if (nodes.length > 2) {
			newNodes.push(...nodes.slice(1, -1))
		}
		// End node; must have two or more nodes:
		if (nodes.length > 1) {
			newNodes.push(nodes.slice(-1)[0] + this.nodes[range.node[1]].slice(range.offset[1]))
		}

		Object.assign(this, {
			data: newData,
			nodes: newNodes,
		})
	}

}

const v = new VDOM("foo\nbar\nbaz")
v.write("abc\ndef\nghi", 2, 9)

// foo
// bar
// baz
//
// abc
// def
// ghi
//
// foabc
// def
// ghiaz
